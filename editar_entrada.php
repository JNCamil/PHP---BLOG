...
<?php $entrada_actual = verEntrada_individual($bd, $_GET['id']);
    ...
    <h3><?= $entrada_actual['titulo'] ?> <span class="badge bg-warning"><svg width="40" height="40" viewBox="0 0 155 155" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<defs>
<clipPath id="clip0">
<rect width="154" height="155" fill="white" transform="translate(0.777344)"/>
</clipPath>
</defs>
</svg></span></h3>
    <br>
    <div class="card  text-bg-secondary mx-auto w-50">
        ...
        <div class="card-body ">
            <form action="guardar_entradas.php?editar=<?=$entrada_actual['id']?>" method="post">
                <!--FLAG desde URL guardar_entradas.php?editar=1-->
                <label for="titulo" class="form-label">Título:</label>
                <input type="text" name="titulo" class="form-control" value="<?= $entrada_actual['titulo'] ?>"><br>
                <?php echo isset($_SESSION["errores_entrada"]) ? mostrarError($_SESSION["errores_entrada"], "titulo") : ""; ?>
                <label for="descripcion" class="form-label">Descripción:</label>
                <textarea name="descripcion" class="form-control" ><?= $entrada_actual['descripcion'] ?></textarea><br>
                ...
                <select name="categoria">
                    ...
                    if (isset($categorias)):
                        foreach ($categorias as $categoria): ?>
                            <option value="<?php echo $categoria["id"] ?>" <?=($categoria['id']==$entrada_actual['categoria_id'])?'selected="selected"':''?> >
